...
function saveClick(callbackFunc) {
    ...
    for(i=0; i<inputs.length; i++){
        ...
        if($(currInput).attr("required") && !$(currInput).val()){
            $(currInput).parent().addClass("has-error");
            errCount++;
        } else {
            $(currInput).parent().removeClass("has-error");
            switch($(currInput).attr('type')){
                case 'checkbox':
                    if($(currInput).prop('checked')==true){
                        if(data[currInput.name] in data){
                            //has key, append
                            data[currInput.name].push($(currInput).val());
                        } else {
                            data[currInput.name] = [$(currInput).val()];
                        }
                    }
                    break;
                case 'radio':
